...
const VideoList: NextPage = () => {
  ...
  if (!isLoading && !video) {
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center bg-black">
        <span className="max-w-[80%] text-center text-2xl font-medium text-white">
          This recording is currently unavailable
        </span>
        <span className="mt-3 max-w-[80%] text-center text-sm text-white/70">
          ...
          <Link
            ...
            href="/sign-in"
            className="pointer text-white underline"
          >
            create an account
          </Link>{" "}
          for free!
  ...
  return (
    <>
      <Head>
        <title>
          {video?.title ?? "Chitram | The Open Source Loom Alternative"}
        </title>
      ...
      <main className="flex h-screen w-screen flex-col items-center justify-center bg-black">
        <div className="flex min-h-[62px] w-full items-center justify-between border-b border-solid border-white/10 bg-black px-6">
          <Link href="/">
            <span className="text-white">Chitram</span>
          </Link>
          <div className="flex items-center justify-center">
            ...
            {status === "authenticated" ? (
              <>
                <Link href="/videos">
                  <span className="cursor-pointer rounded border border-white/20 px-2 py-2 text-sm text-white hover:bg-white/10">
                    My Library
                  </span>
                </Link>
              ...
              <button
                ...
                className="inline-flex max-h-[35px] items-center rounded-md border border-white/20 bg-white px-4 py-2 text-sm font-medium text-black shadow-sm hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black"
              >
                Record a video
              </button>
            )}
        ...
        <div className="flex h-full w-full grow flex-col items-center justify-start overflow-auto bg-black">
            ...
            <div>
              {video?.title ? (
                <div className="mb-4 flex flex-col">
                  <span className="text-[18px] text-lg font-medium text-white">
                    {video.title}
                  </span>
                  <span className="text-[18px] text-sm text-white/60">
                    {getTime(video.createdAt)}
                  </span>
                </div>
              ) : (
                <div className="mb-4 flex flex-col">
                  <div className="h-5 w-[300px] animate-pulse rounded bg-white/10"></div>
                  <div className="mt-2 h-4 w-[50px] animate-pulse rounded bg-white/10"></div>
                </div>
              )}
            </div>
            <div className="mt-2 flex flex-row items-center">
              {!isLoading ? (
                <>
                  ...
                  <span className="ml-3 font-medium text-white">{video.user.name}</span>
                </>
              ) : (
                <>
                  <div className="h-10 w-10 animate-pulse overflow-hidden rounded-full bg-white/10"></div>
                  <div className="ml-3 h-4 w-[100px] animate-pulse rounded bg-white/10 font-medium"></div>
                </>
              )}
            </div>
          </div>
