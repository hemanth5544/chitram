import { type NextPage } from "next";
import Head from "next/head";
import { useSession } from "next-auth/react";
import { useFeatureFlagEnabled, usePostHog } from "posthog-js/react";
import { useAtom } from "jotai";
import recordVideoModalOpen from "~/atoms/recordVideoModalOpen";
import VideoRecordModal from "~/components/VideoRecordModal";
import { ShareIcon } from "@heroicons/react/24/solid";
import { CheckIcon } from "@heroicons/react/20/solid";
import { CheckCircleIcon } from "@heroicons/react/24/outline";
import Footer from "~/components/Footer";
import Header from "~/components/Header";
import Image from "next/image";
import CTA from "~/components/CTA";
import engineeringUsecase from "~/assets/engineering usecase.png";
import supportUsecase from "~/assets/support usecase.png";
import logo from "~/assets/logo.png";
import { useRouter } from "next/router";
import { useEffect } from "react";
import StarIcon from "~/assets/StarIcon";
import Paywall from "~/components/Paywall";
import { motion } from "framer-motion";

const Home: NextPage = () => {
  const [recordModalOpen, setRecordOpen] = useAtom(recordVideoModalOpen);
  const posthog = usePostHog();
  const session = useSession();
  const router = useRouter();
  const showDemoButton = useFeatureFlagEnabled("show-demo-button");

  useEffect(() => {
    if (session.status === "authenticated" && !recordModalOpen) {
      void router.push("/videos");
    }
  }, [session, router]);

  const openRecordModal = () => {
    if (
      !navigator?.mediaDevices?.getDisplayMedia &&
      !navigator?.mediaDevices?.getDisplayMedia
    ) {
      return alert("Your browser is currently NOT supported.");
    }
    setRecordOpen(true);

    posthog?.capture("open record video modal", {
      cta: "landing page",
    });
  };

  return (
    <>
      <Head>
        <title>Chitram | Video Communication Made Simple</title>
        <meta
          name="description"
          content="Create, share, and collaborate with video messages. Fast, secure, and beautifully simple."
        />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className="bg-white">
        <Header />

        <div className="relative isolate px-6 pt-20 lg:px-8 bg-gradient-to-b from-white via-sky-50/30 to-white">
          <div className="mx-auto max-w-7xl py-20 sm:py-32 lg:py-40">
            {/*<div className="hidden sm:mb-8 sm:flex sm:justify-center">*/}
            {/*  <div className="relative rounded-full px-3 py-1 text-sm leading-6 text-gray-600 ring-1 ring-gray-900/10 hover:ring-gray-900/20">*/}
            {/*    Announcing our next round of funding.{" "}*/}
            {/*    <Link href="/blog" className="font-semibold text-indigo-600">*/}
            {/*      <span className="absolute inset-0" aria-hidden="true" />*/}
            {/*      Read more <span aria-hidden="true">&rarr;</span>*/}
            {/*    </Link>*/}
            {/*  </div>*/}
            {/*</div>*/}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, ease: "easeOut" }}
              className="text-center"
            >
              <motion.h1
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2, duration: 0.6 }}
                className="text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-600 via-sky-500 to-sky-700 bg-clip-text text-transparent sm:text-7xl"
              >
                Communicate Better with Video
              </motion.h1>
              <motion.p
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3, duration: 0.6 }}
                className="mt-8 text-xl leading-8 text-gray-700 max-w-2xl mx-auto"
              >
                Record, share, and collaborate effortlessly. No downloads, no complexity—just beautiful video communication that works everywhere.
              </motion.p>
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4, duration: 0.6 }}
                className="mt-10 flex flex-col items-center justify-center gap-6"
              >
                <motion.button
                  whileHover={{ scale: 1.05, boxShadow: "0 10px 25px rgba(14, 165, 233, 0.3)" }}
                  whileTap={{ scale: 0.95 }}
                  onClick={openRecordModal}
                  className="inline-flex h-[70px] flex-col items-center justify-between rounded-lg border border-transparent bg-gradient-to-r from-sky-500 to-sky-600 px-8 py-4 text-sm font-medium text-white shadow-lg transition-all hover:from-sky-600 hover:to-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
                >
                  <span className="text-base font-semibold">Start Recording</span>
                  <span className="text-xs opacity-90">Free to try, no signup needed</span>
                </motion.button>
                <div className="flex flex-col gap-6 sm:flex-row">
                  <motion.a
                    whileHover={{ scale: 1.05, x: 5 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      posthog?.capture("clicked watch recorded demo demo")
                    }
                    target="_blank"
                    href="#"
                    className="text-sm font-semibold leading-6 text-sky-600 transition-colors hover:text-sky-700"
                  >
                    See it in action <span aria-hidden="true">→</span>
                  </motion.a>
                  {showDemoButton ? (
                    <motion.a
                      whileHover={{ scale: 1.05, x: 5 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => posthog?.capture("clicked schedule demo")}
                      target="_blank"
                      href="https://cal.com/hemanth/chitram-demo"
                      className="text-sm font-semibold leading-6 text-sky-600 transition-colors hover:text-sky-700"
                    >
                      Book a demo{" "}
                      <span aria-hidden="true">→</span>
                    </motion.a>
                  ) : null}
                </div>
              </motion.div>
            </motion.div>
          </div>

          <div
            className={`flex w-full items-center justify-center border-t pt-6`}
          >

          </div>

          {/* Stats Section - Moved up */}
          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="flex w-full items-center justify-center border-y border-sky-100 bg-gradient-to-b from-sky-50 to-white my-20"
          >
            <div className="flex max-w-7xl flex-1 flex-col items-center justify-center py-8 lg:h-[140px] lg:flex-row">
              {[
                { stat: "Happy Users", value: "50K+" },
                { stat: "Videos Shared", value: "1M+" },
                { stat: "Uptime", value: "99.9%" },
              ].map(({ stat, value }, index) => (
                <motion.div
                  key={stat}
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1, duration: 0.5 }}
                  whileHover={{ scale: 1.05 }}
                  className="flex flex-1 flex-col py-5 text-center lg:border-r lg:border-sky-100 lg:py-0 lg:last:border-r-0"
                >
                  <span className="text-6xl font-bold bg-gradient-to-r from-sky-600 to-sky-500 bg-clip-text text-transparent">{value}</span>
                  <span className="pt-2 text-sm font-semibold uppercase text-gray-600">
                    {stat}
                  </span>
                </motion.div>
              ))}
            </div>
          </motion.div>

          <div className="mx-auto hidden max-w-7xl py-6 sm:py-6 lg:py-6">
            <div className="text-center">
              <p className="mt-6 text-xl font-semibold leading-8 text-gray-600">
                TRUSTED BY THE BEST BRANDS
              </p>
              <div className="relative flex overflow-x-hidden before:absolute before:left-0 before:z-10 before:h-full before:w-[15vw] before:bg-gradient-to-r before:from-white before:to-transparent after:absolute after:right-0 after:h-full after:w-[15vw] after:bg-gradient-to-r after:from-transparent after:to-white">
                {[1, 2].map((x) => (
                  <div
                    key={x}
                    className={`flex flex-row whitespace-nowrap py-12 ${
                      x === 1
                        ? "animate-marquee"
                        : "absolute top-0 animate-marquee2"
                    }`}
                  >
                    {[
                      {
                        url: "https://producthunt.com/",
                        image:
                          "data:image/svg+xml;base64,",
                        title: "",
                      },
                      {
                        url: "https://samsung.com",
                        image:
                          "data:image/svg+xml,%3Csvg xmlns:inkscape='http://www.inkscape.org/namespaces/inkscape' xmlns:sodipodi='http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd' xmlns='http://www.w3.org/2000/svg' xmlns:svg='http://www.w3.org/2000/svg' version='1.1' id='svg2' xml:space='preserve' width='7051.4023' height='1080' viewBox='0 0 7051.4024 1080' sodipodi:docname='Samsung_wordmark.svg' inkscape:version='1.1 (c68e22c387, 2021-05-23)'%3E%3Csodipodi:namedview id='namedview11' pagecolor='%23ffffff' bordercolor='%23666666' borderopacity='1.0' inkscape:pageshadow='2' inkscape:pageopacity='0.0' inkscape:pagecheckerboard='0' showgrid='false' fit-margin-top='0' fit-margin-left='0' fit-margin-right='0' fit-margin-bottom='0' inkscape:zoom='0.059829465' inkscape:cx='2849.7664' inkscape:cy='1429.0617' inkscape:window-width='1920' inkscape:window-height='1017' inkscape:window-x='1912' inkscape:window-y='-8' inkscape:window-maximized='1' inkscape:current-layer='svg2'/%3E%3Cdefs id='defs6'%3E%3CclipPath clipPathUnits='userSpaceOnUse' id='clipPath16'%3E%3Cpath d='M 0,166.885 H 628.238 V 0 H 0 Z' id='path14'/%3E%3C/clipPath%3E%3C/defs%3E%3Cg id='g8' transform='matrix(12.944053,0,0,-12.944053,-540.03625,1620.0233)'%3E%3Cg id='g10'%3E%3Cg id='g12' clip-path='url(%23clipPath16)'%3E%3Cg id='g18' transform='translate(558.9328,88.5098)'%3E%3Cpath d='m 0,0 v -11.358 h 7.982 v -11.269 c 0.025,-1.007 -0.03,-2.093 -0.203,-2.962 -0.317,-2.102 -2.314,-5.681 -7.98,-5.681 -5.632,0 -7.593,3.579 -7.933,5.681 -0.143,0.869 -0.204,1.955 -0.204,2.962 v 35.593 c 0,1.259 0.085,2.637 0.352,3.68 0.387,1.897 2.068,5.638 7.743,5.638 5.957,0 7.444,-3.944 7.785,-5.638 0.224,-1.122 0.237,-3.004 0.237,-3.004 V 9.32 h 19.613 v 2.555 c 0,0 0.089,2.666 -0.149,5.154 C 25.769,31.638 13.732,36.26 -0.07,36.26 c -13.827,0 -25.62,-4.665 -27.338,-19.231 -0.155,-1.332 -0.392,-3.728 -0.392,-5.154 v -32.742 c 0,-1.426 0.046,-2.53 0.31,-5.136 1.28,-14.207 13.593,-19.243 27.365,-19.243 13.857,0 26.085,5.036 27.387,19.243 0.231,2.606 0.255,3.71 0.286,5.136 V 0 Z m -135.235,34.165 h -19.696 v -57.613 c 0.031,-1.004 0,-2.132 -0.173,-2.959 -0.411,-1.934 -2.05,-5.656 -7.484,-5.656 -5.364,0 -7.046,3.722 -7.426,5.656 -0.197,0.827 -0.222,1.955 -0.197,2.959 v 57.613 h -19.69 V -21.66 c -0.025,-1.439 0.088,-4.379 0.173,-5.149 1.359,-14.547 12.824,-19.27 27.14,-19.27 14.344,0 25.802,4.723 27.186,19.27 0.109,0.77 0.252,3.71 0.167,5.149 z m -180.97,0 -9.825,-60.876 -9.819,60.876 h -31.771 l -1.685,-77.878 h 19.464 l 0.527,72.094 13.392,-72.094 h 19.748 l 13.404,72.094 0.529,-72.094 h 19.513 l -1.742,77.878 z m -117.631,0 -14.426,-77.878 h 21.037 l 10.871,72.094 10.61,-72.094 h 20.891 l -14.366,77.878 z m 367.435,-62.701 -18.34,62.701 h -28.9 v -77.066 h 19.118 l -1.11,64.707 19.696,-64.707 h 27.717 v 77.066 h -19.243 z m -176.838,42.433 c -0.346,1.538 -0.246,3.172 -0.067,4.026 0.557,2.493 2.232,5.212 7.058,5.212 4.498,0 7.135,-2.804 7.135,-7.012 v -4.762 h 19.2 v 5.428 c 0,16.78 -15.044,19.416 -25.936,19.416 -13.718,0 -24.921,-4.522 -26.967,-17.148 -0.541,-3.436 -0.675,-6.486 0.186,-10.378 3.336,-15.743 30.743,-20.31 34.721,-30.266 0.702,-1.886 0.501,-4.291 0.143,-5.708 -0.596,-2.591 -2.339,-5.197 -7.506,-5.197 -4.846,0 -7.763,2.786 -7.763,6.985 l -0.006,7.474 h -20.666 v -5.941 c 0,-17.215 13.484,-22.409 28.007,-22.409 13.909,0 25.397,4.753 27.24,17.637 0.879,6.657 0.216,10.993 -0.137,12.626 -3.22,16.147 -32.431,21.004 -34.642,30.017 m -253.273,0.191 c -0.377,1.57 -0.289,3.227 -0.079,4.091 0.532,2.481 2.217,5.248 7.128,5.248 4.555,0 7.237,-2.831 7.237,-7.073 v -4.82 h 19.425 v 5.471 c 0,16.941 -15.274,19.641 -26.285,19.641 -13.833,0 -25.136,-4.592 -27.204,-17.309 -0.566,-3.491 -0.663,-6.562 0.155,-10.497 3.372,-15.922 31.05,-20.526 35.077,-30.601 0.754,-1.873 0.526,-4.278 0.152,-5.75 -0.639,-2.618 -2.396,-5.261 -7.606,-5.261 -4.865,0 -7.775,2.834 -7.775,7.091 l -0.027,7.494 h -20.898 v -5.955 c 0,-17.412 13.675,-22.648 28.311,-22.648 14.071,0 25.626,4.795 27.511,17.828 0.937,6.718 0.234,11.09 -0.082,12.748 -3.287,16.345 -32.823,21.186 -35.04,30.302' style='fill:%231428a0;fill-opacity:1;fill-rule:nonzero;stroke:none' id='path20'/%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/g%3E%3C/svg%3E",
                        title: "",
                      },
                      {
                        url: "https://stripe.com",
                        image:
                          "data:image/svg+xml;base64,",
                        title: "",
                      },
                      {
                        url: "https://ycombinator.com",
                        image:
                          "data:image/svg+xml;base64,",
                        title: "",
                      },
                    ].map(({ url, image, title }) => (
                      <div
                        key={url}
                        className="mr-8 h-[54px] min-w-[250px] overflow-hidden"
                      >
                        <a href={url} target="_blank">
                          <Image
                            src={image}
                            alt={title}
                            className="h-[54px] w-[250px]"
                            width="250"
                            height="54"
                          />
                        </a>
                      </div>
                    ))}
                  </div>
                ))}
              </div>
              <div className="mt-4 flex flex-row items-center justify-center gap-x-8 overflow-hidden">
                <div className="animate-slide mt-4 flex flex-row items-center justify-center gap-x-8"></div>
              </div>
            </div>
          </div>

          {/* How it Works Section - Redesigned */}
          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="mx-auto max-w-7xl py-20 px-6"
          >
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="flex flex-col text-center mb-16"
            >
              <span className="text-4xl font-bold tracking-tight bg-gradient-to-r from-gray-900 to-sky-600 bg-clip-text text-transparent sm:text-6xl">
                Simple. Fast. Powerful.
              </span>
              <span className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
                Three steps to better communication. No learning curve, no hassle.
              </span>
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
                {[
                  {
                    icon: (
                      <Image
                        src={logo}
                        alt="logo"
                        width={48}
                        height={48}
                        unoptimized
                      />
                    ),
                    title: "Click & Record",
                    description:
                      "One click starts recording. Capture your screen, voice, and face instantly. Works in any browser, no software needed.",
                  },
                  {
                    icon: <ShareIcon className="h-12 w-12 text-sky-500" />,
                    title: "Share Instantly",
                    description:
                      "Get a shareable link in seconds. Anyone can watch—no accounts, no downloads, no barriers. Just click and view.",
                  },
                  {
                    icon: <CheckCircleIcon className="h-12 w-12 text-sky-500" />,
                    title: "Collaborate Seamlessly",
                    description:
                      "Receive feedback, make decisions, and move projects forward faster. All your videos organized in one place.",
                  },
                ].map(({ title, description, icon }, index) => (
                  <motion.div
                    key={title}
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.15, duration: 0.5 }}
                    whileHover={{ scale: 1.03, y: -8 }}
                    className="flex flex-col items-center text-center p-8 rounded-2xl bg-white border border-sky-100 shadow-sm hover:shadow-lg transition-all"
                  >
                    <motion.div
                      whileHover={{ rotate: 360, scale: 1.1 }}
                      transition={{ duration: 0.6 }}
                      className="mb-6 p-4 rounded-full bg-sky-50"
                    >
                      {icon}
                    </motion.div>
                    <h3 className="mb-4 text-2xl font-bold text-gray-900">{title}</h3>
                    <p className="text-gray-600 leading-relaxed">{description}</p>
                  </motion.div>
                ))}
            </div>
          </motion.div>

          {/* Use Cases Section - Redesigned */}
          <motion.div
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="mx-auto max-w-7xl py-20 px-6"
          >
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6 }}
              className="flex flex-col items-center justify-center text-center"
            >
              <span className="text-4xl font-bold tracking-tight bg-gradient-to-r from-gray-900 to-sky-600 bg-clip-text text-transparent sm:text-6xl">
                Built for Every Team
              </span>
              <span className="mt-4 max-w-[75%] text-lg text-gray-600">
                From engineering to customer support, Chitram adapts to how your team works. 
                Replace long emails and confusing threads with clear, visual communication.
              </span>
            </motion.div>
            <div className="mt-16 grid grid-cols-1 gap-8 md:grid-cols-2">
              {[
                {
                  title: "Development Teams",
                  description:
                    "Transform how your engineering team communicates. Make code reviews clearer, document faster, and onboard new members effortlessly.",
                  features: [
                    "Enhanced code review process",
                    "Knowledge base creation",
                    "Async team collaboration",
                  ],
                  image: engineeringUsecase,
                },
                {
                  title: "Customer Success",
                  description:
                    "Deliver exceptional support experiences. Show, don't tell—resolve issues faster with visual explanations that customers love.",
                  features: [
                    "Visual troubleshooting guides",
                    "Self-service documentation",
                    "Faster team training",
                  ],
                  image: supportUsecase,
                },
              ].map(({ title, description, features, image }, index) => (
                <motion.div
                  key={title}
                  initial={{ opacity: 0, y: 40 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.2, duration: 0.5 }}
                  whileHover={{ scale: 1.02, y: -5 }}
                  className="mx-auto max-w-[600px] flex-1 overflow-hidden rounded-xl border border-sky-100 shadow-sm transition-shadow hover:shadow-lg hover:border-sky-200"
                >
                  <div className="flex flex-col pb-2 pt-8">
                    <div className="flex flex-col px-8">
                      <span className="mb-3 text-xl font-semibold">
                        {title}
                      </span>
                      <span className="text-gray-600">{description}</span>
                    </div>
                    <div className="mx-8 mt-4 border-t border-sky-100 pt-4">
                      {features.map((feature, idx) => (
                        <motion.div
                          key={feature}
                          initial={{ opacity: 0, x: -20 }}
                          whileInView={{ opacity: 1, x: 0 }}
                          viewport={{ once: true }}
                          transition={{ delay: index * 0.2 + idx * 0.1 }}
                          className="mb-2 flex flex-row"
                        >
                          <CheckIcon className="h-6 w-6 text-sky-500" />
                          <span className="text-md ml-4">{feature}</span>
                        </motion.div>
                      ))}
                    </div>
                  </div>
                  <Image
                    src={image}
                    width={600}
                    height={575}
                    alt="usecase cover"
                  />
                </motion.div>
              ))}
            </div>
          </motion.div>
        </div>

        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="flex w-full items-center justify-center border-y border-sky-100 bg-gradient-to-b from-sky-50 to-white"
        >
          <div className="flex max-w-7xl flex-1 flex-col items-center justify-center py-4 lg:h-[140px] lg:flex-row">
            {[
              { stat: "Happy Users", value: "50K+" },
              { stat: "Videos Shared", value: "1M+" },
              { stat: "Uptime", value: "99.9%" },
            ].map(({ stat, value }, index) => (
              <motion.div
                key={stat}
                initial={{ opacity: 0, scale: 0.8 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1, duration: 0.5 }}
                whileHover={{ scale: 1.05 }}
                className="flex flex-1 flex-col py-5 text-center lg:border-r lg:border-sky-100 lg:py-0 lg:last:border-r-0"
              >
                <span className="text-6xl font-bold bg-gradient-to-r from-sky-600 to-sky-500 bg-clip-text text-transparent">{value}</span>
                <span className="pt-2 text-sm font-semibold uppercase text-gray-600">
                  {stat}
                </span>
              </motion.div>
            ))}
          </div>
        </motion.div>

        <CTA />

        <Footer />
      </div>

      <VideoRecordModal />
      <Paywall />
    </>
  );
};

export default Home;
